...
<body>
	...
	function main(){
		...
		var cf = confirm("모두 취소하고 전자결재목록 으로 돌아가시겠습니까?");
		if( cf == true){
			location.href="app_line.do";
		}
	}
	...
	function dragover_handler(ev) {
		...
		var idx_check_name2 = ev.target.className;
		var idx_check_str = ev.target.className.substr(11,1);
		var idx_check_number = Number(idx_check_str);
		
		if(idx_check_number >= 2){
			idx_check_name = ".g_position_"+(idx_check_number-1);
			idx_check_name2 = ".g_position_"+(idx_check_number+1);
			if($(idx_check_name2).text() != ""){
				idx_check_back = $(idx_check_name2).attr('id');
			}
		...
		$.ajax({
			...
			success:function(data){
					...
					if(user_check[0].result == "ok"){
						...
						alert("뒤에 등록한 근무자 보다 낮은 직급의 사람만 등록 가능 합니다.");
					}
